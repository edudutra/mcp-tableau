...
- [x] 1.1 Adicionar dependências de runtime ao `pyproject.toml` (`fastmcp`, `tableauserverclient`,
  `pydantic`, `python-dotenv`, `tableaudocumentapi`, `Pillow`, `rapidfuzz`) e dev
  (`pytest`, `pytest-cov`, `ruff`).
- [x] 1.2 Implementar `config.py` com `Settings` (Pydantic `BaseSettings`): URL do servidor,
...
- [x] 1.3 Implementar `models.py` com todos os modelos Pydantic de saída (`PublishResult`,
...
- [x] 1.4 Implementar `server.py` (instancia `FastMCP`, define transporte stdio, ponto único de
  registro das tools) e `main.py` (apenas invoca `server.run()`).
- [x] 1.5 Criar `.env.example` com `TABLEAU_SERVER_URL`, `TABLEAU_SITE`, `TABLEAU_PAT_NAME`,
  `TABLEAU_PAT_SECRET` e limiares opcionais.
- [x] 1.6 Criar estrutura de pacote `src/mcp_tableau/` (`__init__.py`, subpastas `tableau/`,
...
- [x] `test_settings_carrega_variaveis_obrigatorias`
- [x] `test_settings_variavel_faltante_levanta_erro_claro`
- [x] `test_settings_thresholds_default_quando_env_ausente`
- [x] `test_settings_thresholds_override_por_env`
- [x] `test_toolerror_serializa_com_code_e_message`
- [x] `test_publishresult_serializa_campos_obrigatorios`
- [x] `test_models_campos_opcionais_aceitam_null`
